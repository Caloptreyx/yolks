...
## Date: 2024/06/05
## License: MIT License
## Modified for Caloptreyx/yolks: NSS wrapper files are created at runtime, so any container UID works.
...
# Setup NSS Wrapper for use ($NSS_WRAPPER_PASSWD and $NSS_WRAPPER_GROUP are set by the Dockerfile).
# DayZ needs the container UID to resolve to a user name; Wings runs containers with arbitrary UIDs.
export USER_ID=$(id -u)
